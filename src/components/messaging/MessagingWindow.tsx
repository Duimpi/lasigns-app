...
import { MessageSquare, X, Minus, Send, Smile, Users, ArrowLeft, Trash2 } from 'lucide-react'
...
export function MessagingWindow() {
  ...
  async function loadChats() {
    ...
    const { data } = await supabase
      .from('chats')
      .select(`*, members:chat_members(*, profile:profiles(*)), messages:chat_messages(id, content, created_at)`)
      .order('updated_at', { ascending: false })

    const myChats = ((data as any[]) || []).filter(chat =>
    ...
    const chatsWithUnread = await Promise.all(myChats.map(async (chat) => {
      ...
      return { ...chat, unread_count: unread, last_message: lastMsg }
    }))

    setChats(chatsWithUnread as any)
  ...
  async function openDirectChat(targetProfile: Profile) {
    ...
    const existing = (chats as any[]).find(chat =>
      chat.type === 'direct' &&
      chat.members?.some((m: any) => m.profile_id === profile.id) &&
  ...
  async function deleteChat(chatId: string) {
    ...
    if ((activeChat as any)?.id === chatId) setActiveChat(null)
    loadChats()
  }
  ...
  function insertEmoji(emoji: string) {
    ...
    if (input) {
      ...
      const val = newMessage.substring(0, start) + emoji + newMessage.substring(end)
      setNewMessage(val)
      ...
      <AnimatePresence>
        {isOpen && !isMinimized && (
          <motion.div
            ...
            style={{ width: '320px', height: '460px', display: 'flex', flexDirection: 'column' }}
          >
            {/* Header */}
            <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-bg-elevated/60 shrink-0">
              {activeChat ? (
                <div className="flex items-center gap-1.5 min-w-0">
                  <button onClick={() => setActiveChat(null)} className="btn-icon w-6 h-6 shrink-0">
                    <ArrowLeft className="w-3.5 h-3.5" />
                  </button>
                  <p className="text-sm font-semibold text-text-primary truncate">{getChatName(activeChat)}</p>
                </div>
              ) : (
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                <button onClick={() => setIsMinimized(true)} className="btn-icon w-6 h-6"><Minus className="w-3 h-3" /></button>
                <button onClick={() => setIsOpen(false)} className="btn-icon w-6 h-6"><X className="w-3 h-3" /></button>
              </div>
            </div>
            ...
            {!activeChat && (
              <div className="flex border-b border-border shrink-0">
                <button onClick={() => setView('chats')}
                  className={`flex-1 py-1.5 text-xs font-semibold border-b-2 transition-colors ${view === 'chats' ? 'border-accent text-accent' : 'border-transparent text-text-secondary'}`}>
                  Chats
                </button>
                <button onClick={() => setView('team')}
                  className={`flex-1 py-1.5 text-xs font-semibold border-b-2 transition-colors ${view === 'team' ? 'border-accent text-accent' : 'border-transparent text-text-secondary'}`}>
              ...
              <div className="flex-1 overflow-y-auto">
                {(chats as any[]).length === 0 ? (
                  <div className="py-8 text-center">
                    <MessageSquare className="w-7 h-7 text-text-muted mx-auto mb-2 opacity-30" />
                    <p className="text-xs text-text-muted mb-1">No chats yet</p>
                    <button onClick={() => setView('team')} className="text-xs text-accent">
                  ...
                  (chats as any[]).map(chat => (
                    <div key={chat.id} onClick={() => openChat(chat)}
                      className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-bg-hover cursor-pointer border-b border-border/30 group">
                      <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center shrink-0">
                        {chat.type === 'group'
                          ? <Users className="w-3.5 h-3.5 text-accent" />
                          : <span className="text-xs font-bold text-accent">{getChatName(chat)[0]?.toUpperCase()}</span>}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {chat.last_message && (
                          <p className="text-[10px] text-text-muted truncate">{chat.last_message.content}</p>
                        )}
                      </div>
            ...
            {!activeChat && view === 'team' && (
              <div className="flex-1 overflow-y-auto">
                <p className="text-[10px] text-text-muted px-3 py-2 border-b border-border/40">
                  Tap a name to start a direct message
                </p>
                {teamMembers.length === 0 ? (
                  <div className="py-6 text-center text-xs text-text-muted">Loading team...</div>
                ) : (
            ...
            {activeChat && (
              <>
                <div className="flex-1 overflow-y-auto p-3 space-y-2">
                  {messages.length === 0 && (
                    <p className="text-center text-xs text-text-muted py-6">No messages yet — say hi!</p>
                  )}
                  {messages.map(msg => {
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] flex flex-col gap-0.5 ${isOwn ? 'items-end' : 'items-start'}`}>
                          ...
                          <div className={`px-3 py-1.5 rounded-xl text-sm leading-snug ${
                ...
                <div className="px-2 py-2 border-t border-border shrink-0 relative">
                  ...
                  <div className="flex gap-1.5 items-center">
                    <button onClick={() => setShowEmojiPicker(!showEmojiPicker)}
                      className={`btn-icon w-7 h-7 shrink-0 ${showEmojiPicker ? 'text-accent' : ''}`}>
                    ...
                    <input ref={inputRef} value={newMessage}
                      ...
                      className="input flex-1 py-1.5 text-sm" placeholder="Type a message..." />
                    <button onClick={sendMessage} disabled={isSending || !newMessage.trim()}
                      className="btn-primary w-7 h-7 p-0 shrink-0">
                      {isSending ? <span className="spinner w-3 h-3" /> : <Send className="w-3 h-3" />}
                    </button>
                  </div>
                </div>
              </>
